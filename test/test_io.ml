...
let tests = [
  ...
  ( "Trx.retry" >:: (fun _ ->
    ...
    let trx = Io.Trx.empty
    |> Io.Trx.add (Io.Action.of_write_file (Filename.concat test_dir "testfile01.txt") (Core.Sexp.of_string "TEST01"))
    |> Io.Trx.add (Io.Action.of_delete_file (Filename.concat test_dir "testfile03.txt"))
    in
    ...
    assert_raises (Io.TransactionError "transaction already exists: ./test_io_trx/trx.hit") (fun () ->
       Io.execute_transaction trx_path trx
    );
