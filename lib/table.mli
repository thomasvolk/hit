...
module DocumentTable : sig
  (** A module representing a document table that maps documents to
      token entries and their positions. *)
  ...
  (** [add document_id (flags, pos_list) document_table] adds a new entry to
      the [document_table] for the given [document_id] with the specified
      [flags] and list of positions [pos_list] *)
  ...
  (** [all document_table] returns a list of all entries in the
      [document_table] as (document_id, (flags, pos_list)) pairs *)
...
module TokenTable : sig
  (** A module representing a token table that maps tokens to
      document table IDs. *)
  ...
  (** [add token dt_id token_table] adds a mapping from the given
      [token] to the specified [dt_id] in the [token_table] *)

  val get : TokenMap.key -> 'a TokenMap.t -> 'a option
  (** [get token token_table] retrieves the document table ID associated
      with the given [token] from the [token_table], returning [None] if it
      does not exist *)
  ...
  (** [to_list token_table] converts the [token_table] to a list of
      (token, document_table_id) pairs *)

  val empty : 'a TokenMap.t
  (** [empty] creates an empty token table *)

  val size : 'a TokenMap.t -> int
  (** [size token_table] returns the number of entries in the
      [token_table] *)

  val merge : 'a TokenMap.t -> 'a TokenMap.t -> 'a TokenMap.t
  (** [merge token_table_a token_table_b] merges two token tables,
      preferring entries from [token_table_a] in case of conflicts *)
end
