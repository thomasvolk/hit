...
First you have to initialize the index with a given path:

#+begin_src ocaml :exports both :results silent :tangle example.index-api.utop
  #require "hit";;
  open Hit
  let index = Index.open "./my_index";;
#+end_src
...
#+begin_src ocaml :exports both :results pp :tangle example.index-api.utop
  ...
  List.iter ~f:(fun did -> Printf.printf "Found document via query: %s\n" did) results;;
#+end_src
...
#+begin_src ocaml :exports both :results silent :tangle example.index-api.utop
  Index.delete delete "27b3a15d450986a691a78be2276c5907";;
#+end_src
