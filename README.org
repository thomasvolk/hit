...
#+begin_src ocaml :exports both :results silent
    let index = index
        ...
        |> Idx.add_doc (Document.from_source "local" "/documents/doc4.txt" "This is the content of document four.")
#+end_src

add_doc allways parses thne document content and updates the index. To check first if the document already existss
and only update it if the content has changed, use update_doc:

#+begin_src ocaml :exports both :results silent
    let index = index
        |> Idx.update_doc (Document.from_source "local" "/documents/doc1.txt" "This is the content of document one.")
        |> Idx.update_doc (Document.from_source "local" "/documents/doc2.txt" "This is the content of document II.")
#+end_src
