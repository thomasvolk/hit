...
#+RESULTS:
#+begin_example
module Storage : Hit.Io.StorageInstance
module Idx :
  sig
    val load : unit -> Hit.Index.t
    val init : unit -> unit
    val get_doc : Hit__.Document.Id.t -> Hit__.Document.t
    val add_doc : Hit__.Document.t -> Hit.Index.t -> Hit.Index.t
    val update_doc : Hit__.Document.t -> Hit.Index.t -> Hit.Index.t
    val delete_doc : Hit__.Document.Id.t -> Hit.Index.t -> Hit.Index.t
    val get_entries :
      Hit.Index.t ->
      string -> (Hit__.Document.Id.t * Hit__.Text.TokenEntry.t list) list
    val find_entries :
      Hit.Index.t ->
      (string -> bool) ->
      (Hit__.Document.Id.t * Hit__.Text.TokenEntry.t list) list
    val flush :
      ?clear_cache:bool -> ?force:bool -> Hit.Index.t -> Hit.Index.t
    val garbage_collect : Hit.Index.t -> Hit.Index.t
  end
#+end_example
...
module Q :
  sig
    val query :
      Hit.Index.Query.t -> Hit.Index.t -> Hit.Index.QueryResult.t list
    val find_docs :
      string list -> Hit.Index.t -> Hit.Index.QueryResult.t list
  end
val results : Hit.Index.QueryResult.t list =
  [{Hit.Index.QueryResult.doc_id = "620e6c2c1fb0eaaabb6061f7daa5f98d";
    token_entries =
     [{Hit__.Text.TokenEntry.token = "one"; positions = [32];
       flags =
        {Hit__.Text.TokenEntry.Flags.title = false; directory = false;
         extension = false; source = false}}]}]
#+end_example
